...
d3.selectAll('.brush>.handle--n').remove();
...
function handleMouseOverEst(d, i) {
	...
	tool_tip.show(data[i], document.getElementById("bar" + i));
...
function handleMouseOutEst(d, i) {
	tool_tip	.hide(data[i], document.getElementById("bar" + i));
	for(j=0;j<data.length;j++){
		if (i != j) {
